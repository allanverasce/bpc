...
@Slf4j
public class SendFileFunction implements CallableFunction {

    /**
     * 
     * This class implements the {@link CallableFunction} interface and represents
     * the function that is responsible for sending a file to the user.<br>
     * <br>
     * The execution of this function will send the file to the user and set the
     * message status to {@link MessageStatus#DOWNLOADED}. <br>
     * <br>
    ...
    public void execute(FunctionContext context) {
        ...
        ObjectInputStream input = context.getInput();
        try {
            bot.ifPresent(b -> b.notifyProjectStarted(message));
            FileUtils.mountFile(input, message);
            message.setMessageStatus(MessageStatus.DOWNLOADED);
            bot.ifPresent(b -> b.notifyFinishDownloadFiles(message));
        } catch (Exception e) {
            log.error("Error to send file", e);
            bot.ifPresent(b -> b.notifyErrorDownloadFiles(message));
