...
@Slf4j
public class SaveProjectFunction implements CallableFunction {

    /**
     * 
     * This class implements the {@link CallableFunction} interface and represents
     * the function that is responsible for saving a project in the database.<br>
     * <br>
     * The execution of this function will save the project in the database and set
    ...
    public void execute(FunctionContext context) {
        ...
        try {
            ...
            ProjectDto projectSaved = new ModelMapper().map(project, ProjectDto.class);
            message.addParameter(ParameterType.PROJECT, projectSaved);
            message.setMessageStatus(MessageStatus.PROCESSED);
        } catch (Exception e) {
            log.error("Error to save project", e);
